...
const clientType = "flutter"; // This must match the ClientType.FLUTTER in the server
const clientId = "test-flutter-client-" + Date.now();
...
ws.on("open", () => {
  ...
  const testMessage = {
    id: `test-${Date.now()}`,
    method: "flutter.testMethod",
    params: {
      testParam: "Hello from Flutter test client",
    },
    jsonrpc: "2.0",
  };
  ...
  try {
    ...
    if (message.method && message.id) {
      const response = {
        ...
        result: {
          status: "success",
          message: `Flutter client processed ${message.method}`,
        },
        jsonrpc: "2.0",
      };
...
console.log("Test Flutter client running. Press Ctrl+C to stop.");
