...
import 'package:devtools_mcp_extension/core/devtools_core/shared/primitives/ansi_utils.dart';
import 'package:devtools_mcp_extension/core/devtools_core/shared/primitives/byte_utils.dart';
import 'package:devtools_mcp_extension/core/devtools_core/shared/primitives/simple_items.dart';
...
enum DurationDisplayUnit {
  ...
  static DurationDisplayUnit unitFor(final int micros) {
    if (micros < 100) {
...
T? nullSafeMin<T extends num>(final T? a, final T? b) {
  if (a == null || b == null) {
    return a ?? b;
  }
  return min<T>(a, b);
...
T? nullSafeMax<T extends num>(final T? a, final T? b) {
  if (a == null || b == null) {
    return a ?? b;
  }
  return max<T>(a, b);
...
double logBase({required final int x, required final int base}) =>
    log(x) / log(base);

int log2(final num x) => logBase(x: x.floor(), base: 2).floor();

int roundToNearestPow10(final int x) =>
    pow(10, logBase(x: x, base: 10).ceil()).floor();

void executeWithDelay(
  final Duration delay,
  final void Function() callback, {
  final bool executeNow = false,
}) {
...
Future<void> delayToReleaseUiThread({final int micros = 0}) async {
...
Future<T?> timeout<T>(final Future<T> operation, final int timeoutMillis) =>
    Future.any<T?>([
      operation,
      Future<T?>.delayed(
...
String longestFittingSubstring(
  ...
  final num maxWidth,
  final List<num> asciiMeasurements,
  final num Function(int value) slowMeasureFallback,
) {
...
bool isLetter(final int codeUnit) =>
...
/// streams fire.
Stream combineStreams(final Stream a, final Stream b, final Stream c) {
...
class DelayedTimer {
  ...
  void invoke(final VoidCallback closure) {
    _closure = closure;
...
class JsonUtils {
  ...
  static int getIntMember(
    final Map<String, Object?> json,
    final String memberName,
  ) => json[memberName] as int? ?? -1;
}
...
class RateLimiter {
  RateLimiter(final double requestsPerSecond, this.callback)
...
class TimeRange {
  ...
  factory TimeRange.offset({
    required final TimeRange original,
  ...
  set start(final Duration? value) {
    if (singleAssignment) {
      assert(_start == null);
    }
  ...
  bool contains(final Duration target) => start! <= target && end! >= target;

  bool containsRange(final TimeRange t) => start! <= t.start! && end! >= t.end!;

  bool overlaps(final TimeRange t) => t.end! > start! && t.start! < end!;
...
String formatDateTime(final DateTime time) =>
    DateFormat('H:mm:ss.S').format(time);
...
class Reporter implements Listenable {
  ...
  @override
  void addListener(final VoidCallback callback) {
    _listeners.add(callback);
  }

  /// Removes the listener [callback].
  @override
  void removeListener(final VoidCallback callback) {
...
String toStringAsFixed(final double num, [final int fractionDigit = 1]) =>
    num.toStringAsFixed(fractionDigit);

extension SafeListOperations<T> on List<T> {
  T? safeGet(final int index) =>
      index < 0 || index >= length ? null : this[index];

  T? safeRemoveLast() => isNotEmpty ? removeLast() : null;

  List<T> safeSublist(final int start, [final int? end]) {
...
class Range {
  ...
  bool contains(final num target) => target >= begin && target <= end;
...
List<TextSpan> textSpansFromAnsi(
  ...
  return parser.parse().map((final entry) {
...
Color? ansiToColor(final List<int>? ansiInput) {
  if (ansiInput == null) {
    return null;
  }
...
extension LogicalKeySetExtension on LogicalKeySet {
  ...
  String describeKeys({final bool isMacOS = false}) {
    ...
    return sortedKeys.map((final key) {
...
extension StringExtension on String {
  bool caseInsensitiveContains(final Pattern? pattern) {
  ...
  /// [this.length].
  bool caseInsensitiveEquals(final Pattern? pattern) {
    if (pattern is RegExp) {
      assert(!pattern.isCaseSensitive);
      final completeMatch = pattern
          .allMatches(this)
          .firstWhereOrNull(
            (final match) => match.start == 0 && match.end == length,
          );
  ...
  Iterable<Match> caseInsensitiveAllMatches(final String? query) {
...
extension IterableExtension<T> on Iterable<T> {
  ...
  String joinWithTrailing([final String separator = '']) {
...
extension ListExtension<T> on List<T> {
  List<T> joinWith(
    final T separator, {
    final bool includeTrailing = false,
    final bool includeLeading = false,
  }) => [
  ...
  List<int> allIndicesWhere(final bool Function(T element) test) {
    final indices = <int>[];
...
extension SetExtension<T> on Set<T> {
  bool containsAny(final Iterable<T> any) {
    for (final e in any) {
      if (contains(e)) {
        return true;
      }
...
double safePositiveDouble(final double value) {
...
String prettyTimestamp(final int? timestamp, {final bool isUtc = false}) {
...
extension BoolExtension on bool {
  int boolCompare(final bool other) {
...
String? fileNameFromUri(final String? uri) => uri?.split('/').lastOrNull;
...
Map<K, R> subtractMaps<K, F, S, R>({
  ...
  required final R? Function({required S? subtract, required F? from})
  subtractor,
}) {
  from ??= <K, F>{};
  subtract ??= <K, S>{};
