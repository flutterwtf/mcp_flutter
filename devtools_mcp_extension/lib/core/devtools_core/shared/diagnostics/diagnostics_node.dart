...
class RemoteDiagnosticsNode extends DiagnosticableTree {
  ...
  static BoxParentData deserializeParentData(final Map<String, Object?> json) =>
      BoxParentData()
        ..offset = Offset(
  ...
  static Size? deserializeSize(final Map<String, Object> json) {
  ...
  static FlexFit deserializeFlexFit(final String? flexFit) {
  ...
  @visibleForTesting
  static int jsonHashCode(final Map<String, dynamic> json) =>
  ...
  bool get hasCreationLocation =>
      _creationLocation != null || json.containsKey('creationLocation');

  /// Location id compatible with rebuild location tracking code.
  int get locationId => JsonUtils.getIntMember(json, 'locationId');

  set creationLocation(final InspectorSourceLocation? location) {
  ...
  DiagnosticLevel get defaultLevel =>
      getLevelMember('defaultLevel', DiagnosticLevel.info);
  ...
  bool get isDiagnosticableValue =>
      getBooleanMember('isDiagnosticableValue', false);

  String? getStringMember(final String memberName) =>
      JsonUtils.getStringMember(json, memberName);

  bool getBooleanMember(final String memberName, final bool defaultValue) {
  ...
  RemoteDiagnosticsNode? layoutRootNode({required final bool forFlexLayout}) {
  ...
  DiagnosticLevel getLevelMember(
    ...
    final DiagnosticLevel defaultValue,
  ) {
  ...
  DiagnosticsTreeStyle getStyleMember(
    final String memberName,
    final DiagnosticsTreeStyle defaultValue,
  ) {
  ...
  bool isEnumProperty() => type?.startsWith('EnumProperty<') ?? false;
  ...
  bool get childrenReady =>
      json.containsKey('children') || _children != null || !hasChildren;
  ...
  set hideableGroupLeader(final RemoteDiagnosticsNode? newLeader) {
    _hideableGroupLeader = newLeader;
  }

  bool get groupIsHidden => inHideableGroup && _groupIsHidden;

  var _groupIsHidden = true;

  set groupIsHidden(final bool newValue) {
  ...
  bool get isHideableGroupLeader =>
      inHideableGroup && _hideableGroupSubordinates != null;
  ...
  void addHideableGroupSubordinate(final RemoteDiagnosticsNode subordinate) {
  ...
  void toggleHiddenGroup() {
    ...
    if (isHideableGroupLeader) {
      _hideableGroupSubordinates?.forEach(
        (final node) => node.groupIsHidden = newHiddenValue,
      );
    }
  }

  bool _alwaysVisible(final RemoteDiagnosticsNode node) {
  ...
  Future<List<RemoteDiagnosticsNode>> _getChildrenHelper() =>
      objectGroupApi!.getChildren(valueRef, isSummaryTree, this);
  ...
  ) async => objectGroup.getProperties(valueRef);
  ...
  bool identicalDisplay(final RemoteDiagnosticsNode node) {
  ...
  @override
  String toStringShort() => description ?? '';

  Future<void> setSelectionInspector(final bool uiAlreadyUpdated) async {
    final objectGroup = objectGroupApi;
