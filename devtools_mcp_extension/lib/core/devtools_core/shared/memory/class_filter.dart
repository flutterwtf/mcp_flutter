...
class ClassFilter with Serializable {
  ClassFilter({
    ...
    required final String except,
    required final String? only,
  ...
  factory ClassFilter.fromJson(final Map<String, dynamic> json) {
    final type = json[_Json.type] as String?;
    return ClassFilter(
      filterType:
          ClassFilterType.values.lastWhereOrNull((final t) => t.name == type) ??
          _defaultFilterType,
  ...
  static String _trimByLine(final String value) =>
      value.split('\n').map((final e) => e.trim()).join('\n');
  ...
  Set<String> _filtersAsSet() {
    Set<String> stringToSet(final String? s) =>
        s == null
            ? {}
            : s
                .split(RegExp(',|\n'))
  ...
  @visibleForTesting
  FilteringTask task({required final ClassFilter? previous}) {
  ...
  bool apply(final HeapClassName className, final String? rootPackage) {
  ...
  static List<T> filter<T>({
    required final ClassFilter? oldFilter,
    required final List<T>? oldFiltered,
    required final ClassFilter newFilter,
    required final List<T> original,
    required final HeapClassName Function(T) extractClass,
    required final String? rootPackage,
  }) {
    ...
    final result =
        dataToFilter
            .where((final e) => newFilter.apply(extractClass(e), rootPackage))
            .toList();
