...
class HeapClassName with Serializable {
  @visibleForTesting
  HeapClassName({required final String? library, required this.className})
    : library = _normalizeLibrary(library);

  factory HeapClassName.fromJson(final Map<String, dynamic> json) =>
      HeapClassName(
  ...
  static HeapClassName fromClassRef(final ClassRef? classRef) => fromPath(
    library: _library(classRef?.library?.name, classRef?.library?.uri),
    className: classRef?.name ?? '',
  );

  static HeapClassName fromHeapSnapshotClass(
    final HeapSnapshotClass? theClass,
  ) => fromPath(
    library: _library(theClass?.libraryName, theClass?.libraryUri.toString()),
    className: theClass?.name ?? '',
  );

  static String _library(final String? libName, final String? libUrl) {
  ...
  /// See [isWeak].
  static bool _isWeak(final String className, final String library) {
  ...
  ClassType classType(final String? rootPackage) =>
      _cachedClassType ??= _classType(rootPackage);

  ClassType _classType(final String? rootPackage) {
  ...
  static bool _isDartOrFlutter(final String library) {
  ...
  static String _normalizeLibrary(final String? library) =>
      (library ?? '').trim().replaceFirst(
        RegExp('^${PackagePrefixes.dartInSnapshot}'),
        PackagePrefixes.dart,
      );

  bool matches(final ClassRef ref) => HeapClassName.fromClassRef(ref) == this;
