...
class VmServiceWrapper extends VmService {
  VmServiceWrapper(
    ...
    super.streamClosed,
    super.wsUri,
    final bool trackFutures = false,
  }) : _trackFutures = trackFutures {
    unawaited(_initSupportedProtocols());
  }

  static VmServiceWrapper defaultFactory({
    required final Stream<dynamic> /*String|List<int>*/ inStream,
    required final void Function(String message) writeMessage,
    final Log? log,
    final DisposeHandler? disposeHandler,
    final Future? streamClosed,
    final String? wsUri,
    final bool trackFutures = false,
  }) => VmServiceWrapper(
    inStream,
  ...
  Future<void> _initSupportedProtocols() async {
    ...
    final ddsProtocol = supportedProtocols.protocols?.firstWhereOrNull(
      (final p) => p.protocolName?.caseInsensitiveEquals('DDS') ?? false,
    );
  ...
  ).then((final r) => r as AllocationProfile);

  @override
  Future<CpuSamples> getCpuSamples(
    final String isolateId,
    final int timeOriginMicros,
  ...
  Future<HeapSnapshotGraph> getHeapSnapshotGraph(
    ...
    final bool calculateReferrers = true,
    final bool decodeObjectData = true,
    final bool decodeExternalProperties = true,
    final bool decodeIdentityHashCodes = true,
  }) async => HeapSnapshotGraph.getSnapshot(
    this,
    isolateRef,
  ...
  @override
  Future<Success> streamCancel(final String streamId) {
  ...
  @override
  Future<Success> streamListen(final String streamId) {
    if (!_activeStreams.containsKey(streamId)) {
      return _activeStreams[streamId] = super.streamListen(streamId);
    } else {
      return _activeStreams[streamId]!.then((final value) => value);
    }
  }
  ...
  Stream<Event> _maybeReturnStreamWithHistory(
    final Stream<Event> ddsStream, {
    required final Stream<Event> fallbackStream,
  }) {
  ...
  Future<bool> isSocketProfilingAvailableWrapper(final String isolateId) =>
      isSocketProfilingAvailable(isolateId);

  Future<SocketProfilingState> socketProfilingEnabledWrapper(
    final String isolateId, [
    final bool? enabled,
  ]) => socketProfilingEnabled(isolateId, enabled);

  Future<Success> clearSocketProfileWrapper(final String isolateId) =>
      clearSocketProfile(isolateId);

  Future<SocketProfile> getSocketProfileWrapper(final String isolateId) =>
      getSocketProfile(isolateId);

  Future<HttpProfileRequest> getHttpProfileRequestWrapper(
    final String isolateId,
    final String id,
  ) => getHttpProfileRequest(isolateId, id);
  ...
  Future<Success> clearHttpProfileWrapper(final String isolateId) =>
      clearHttpProfile(isolateId);

  Future<bool> isHttpTimelineLoggingAvailableWrapper(final String isolateId) =>
      isHttpTimelineLoggingAvailable(isolateId);

  Future<HttpTimelineLoggingState> httpEnableTimelineLoggingWrapper(
    final String isolateId, [
    final bool? enabled,
  ]) => httpEnableTimelineLogging(isolateId, enabled);
  ...
  @override
  Future<T> wrapFuture<T>(final String name, final Future<T> future) {
    ...
    localFuture.then(
      (final value) => futureComplete(),
      onError: (final error) => futureComplete(),
    );
    return localFuture;
  }
  ...
  Future<InstanceRef?> getReachableSize(
    final String isolateId,
  ...
  Future<dap.ScopesResponseBody?> dapScopesRequest(
    final dap.ScopesArguments args,
  ) async {
  ...
  Future<dap.StackTraceResponseBody?> dapStackTraceRequest(
    final dap.StackTraceArguments args,
  ) async {
